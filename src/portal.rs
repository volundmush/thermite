...
impl Portal {
    ...
    pub async fn run(&mut self) {
        let mut interval_timer = IntervalStream::new(time::interval(Duration::from_millis(5000)));

        while self.running {
            tokio::select! {
                f_msg = self.rx_portal.recv() => {
                    if let Some(msg) = f_msg {
                        self.handle_msg2portal(msg).await;
                    }
                },
    ...
    async fn handle_msg2portal(&mut self, msg: Msg2Portal) {
        match msg {
            Msg2Portal::Kill => {
                self.running = false;
            },
